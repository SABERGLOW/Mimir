...
function Post({ post, postId }: Props) {
    ...
    */
	...
	const upVote = async (isUpvote: boolean) => {
		if (!session) {
			toast("You must be logged in to vote.", {
				icon: "❗",
				className: "dark:bg-[#3A9188]  dark:text-[#F1F2EB]",
			});
			return;
		}
	...
	/**
	 * It returns a boolean value that indicates whether the URL is an image or not.
	 * @see {@link https://www.zhenghao.io/posts/verify-image-url#sending-head-requests-and-only-check-for-the-mime-type}
	 * @param {string} url - The URL of the image you want to check.
	 * @returns A promise that resolves to a boolean.
	 */
	/**
     * Create a new promise, create a new image, set the image's onerror to resolve false, set the image's
     * onload to resolve true, set the image's src to the url, and return the promise.
	...
	return (
		<div className='flex cursor-pointer border rounded-md border-gray-300 bg-white shadow-sm hover:border hover:border-gray-600 dark:bg-gradient-to-b dark:from-[#062925] dark:to-[#044A42] font-gilroy subpixel-antialiased backdrop-blur-sm dark:hover:border-[#3A9188] dark:border-[#062925]'>
			...
			<div className='flex flex-col items-center justify-start space-y-1 rounded-l-md bg-gray-50 p-4 text-gray-400 dark:bg-[#062925] font-gilroy subpixel-antialiased'>
				...
				<ArrowUpIcon
					onClick={() => upVote(true)}
					className={`voteButtons stroke-[4px] hover:text-red-400 dark:hover:text-[#3A9188] ${
						vote && "text-red-400 dark:text-[#3A9188]"
					}`}
				/>

				{/* if {displayVotes(data)} is less than zero, display red text, otherwise display green text */}
				<p
					className={`text-black font-bold text-sm subpixel-antialiased ${
						displayVotes(data) < 0
							? "dark:text-[#F05454]"
							: "dark:text-[#B8E1DD]"
					} `}
				...
				<ArrowDownIcon
					...
					className={`voteButtons stroke-[4px] hover:text-red-400 dark:hover:text-[#F05454] ${
						vote === false && "text-blue-400 dark:text-[#F05454]"
					}`}
				/>
			</div>
